package com.warriormedia.library.orm;
...
public abstract class DataManager {
    ...
    public <T extends BaseModel> T get(Class<T> type, boolean distinct, String[] selectColumns, String whereClause,
        ...
        Cursor c = null;
        try {
            c = mDatabase.query(false, com.warriormedia.library.orm.Utils.getTableName(type), selectColumns, whereClause, whereArgs, groupBy,
                    having, orderBy, limit);
            if (c.moveToNext()) {
                entity = type.newInstance();
                entity = com.warriormedia.library.orm.Utils.inflate(c, entity);
            }
    ...
    public <T extends BaseModel> List<T> getList(Class<T> type, boolean distinct, String whereClause,
        ...
        Cursor c = mDatabase.query(distinct, com.warriormedia.library.orm.Utils.getTableName(type), null, whereClause, whereArgs, groupBy, having,
                orderBy, limit);
    ...
    public <T extends BaseModel, T2 extends BaseModel> List<T> getList(boolean distinct, String whereClause,
        ...
        try {
            ...
            String returnTableName = com.warriormedia.library.orm.Utils.getTableName(type1);
            for (int i = 0; i < columns.size(); ++i) {
                Field column = columns.get(i);
                column.setAccessible(true);
                String colName = com.warriormedia.library.orm.Utils.toSQLName(column.getName());
                if (i != columns.size() - 1) {
            ...
            // for table_name
            sql.append(com.warriormedia.library.orm.Utils.getTableName(type1)).append(DOT);
            sql.append(com.warriormedia.library.orm.Utils.getTableName(type2));
    ...
    public <T extends BaseModel> int countColumn(Class<T> type, boolean distinct, String[] column) {
        try {
            Cursor cursor = mDatabase.query(distinct, com.warriormedia.library.orm.Utils.getTableName(type), column, null, null, null, null, null,
                    null);
        ...
        for (Field column : columns) {
            try {
                ...
                if (fieldValue != null
                    ...
                    values.put(com.warriormedia.library.orm.Utils.toSQLName(column.getName()), String.valueOf(fieldValue));
                } else if (fieldValue != null
                        && (com.warriormedia.library.orm.Utils.isInstanceofBaseModel(column.getType()) || typeString.equals("java.util.ArrayList") || typeString
                        .equals("java.util.List"))) {
                    // 如果该字段继承自BaseModel；也可以保存到数据库中，转化为Json字符串保存
                    values.put(com.warriormedia.library.orm.Utils.toSQLName(column.getName()), new Gson().toJson(fieldValue));
                }
            } catch (IllegalAccessException e) {
                throw new DataAccessException(e.getLocalizedMessage());
            }
        ...
        for (Field column : columns) {
            try {
                ...
                if (fieldValue != null
                    ...
                    // 如果没有只，用空覆盖
                    values.put(com.warriormedia.library.orm.Utils.toSQLName(column.getName()), "");
                }
    ...
    public <T extends BaseModel> int updateByClause(Class<T> type, ContentValues values, String whereClause,
        ...
        int rowAffect = 0;
        try {
            String table = com.warriormedia.library.orm.Utils.getTableName(type);
    ...
    public <T extends BaseModel> boolean delete(Class<T> type, String whereClause, String[] whereArgs) {
        return delete(com.warriormedia.library.orm.Utils.getTableName(type), whereClause, whereArgs);
    }
