...
import com.warriormedia.library.imageloader.utils.FileUtils;
...
final class LoadAndDisplayImageTask implements Runnable {
    ...
    private final com.warriormedia.library.imageloader.core.ImageLoaderConfiguration configuration;
    private final ImageLoadingInfo imageLoadingInfo;
    private final Handler handler;

    public LoadAndDisplayImageTask(com.warriormedia.library.imageloader.core.ImageLoaderConfiguration configuration, ImageLoadingInfo imageLoadingInfo,
                                   Handler handler) {
    ...
    public void run() {
        if (configuration.loggingEnabled)
            Log.i(com.warriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_START_DISPLAY_IMAGE_TASK, imageLoadingInfo.memoryCacheKey));
        ...
        if (imageLoadingInfo.options.isCacheInMemory()) {
            if (configuration.loggingEnabled)
                Log.i(com.warriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_CACHE_IMAGE_IN_MEMORY, imageLoadingInfo.memoryCacheKey));
        ...
        if (checkTaskIsNotActual())
            return;
        if (configuration.loggingEnabled)
            Log.i(com.warriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_DISPLAY_IMAGE_IN_IMAGEVIEW, imageLoadingInfo.memoryCacheKey));

        DisplayBitmapTask displayBitmapTask = new DisplayBitmapTask(
                bmp, imageLoadingInfo.imageView, imageLoadingInfo.options.getDisplayer(), imageLoadingInfo.listener);
    ...
    boolean checkTaskIsNotActual() {
        String currentCacheKey = com.warriormedia.library.imageloader.core.ImageLoader.getInstance().getLoadingUriForView(imageLoadingInfo.imageView);
    ...
    private Bitmap tryLoadBitmap() {
        ...
        try {
            ...
            if (imageFile.exists()) {
                if (configuration.loggingEnabled)
                    Log.i(com.warriormedia.library.imageloader.core.ImageLoader.TAG,
                            String.format(LOG_LOAD_IMAGE_FROM_DISC_CACHE, imageLoadingInfo.memoryCacheKey));
                ...
                // 缺陷原因：清除图片后会把图片缓存目录也会删除，导致再次加载时找不到目录
                // 解决方案：重新加载时，判断缓存目录是否存在，不存在就重新生成目录
                FileUtil.prepareFile(imageFile.getParent());
                imageFile.createNewFile();
            }

            // Load image from Web
            if (configuration.loggingEnabled)
                Log.i(com.warriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_LOAD_IMAGE_FROM_INTERNET, imageLoadingInfo.memoryCacheKey));

            URI imageUriForDecoding;
            if (imageLoadingInfo.options.isCacheOnDisc()) {
                if (configuration.loggingEnabled)
                    Log.i(com.warriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_CACHE_IMAGE_ON_DISC, imageLoadingInfo.memoryCacheKey));
            ...
            Log.e(com.warriormedia.library.imageloader.core.ImageLoader.TAG, e.getMessage(), e);
            fireImageLoadingFailedEvent(FailReason.IO_ERROR);
            if (imageFile.exists()) {
                imageFile.delete();
            }
    ...
    private Bitmap decodeImage(URI imageUri) throws IOException {
        ...
        if (configuration.handleOutOfMemory) {
            ...
            bmp = decoder.decode(imageLoadingInfo.targetSize, imageLoadingInfo.options.getImageScaleType(),
                    imageLoadingInfo.imageView.getScaleType());
        }
        return bmp;
    }
    ...
    private Bitmap decodeWithOOMHandling(URI imageUri) throws IOException {
        ...
        for (int attempt = 1; attempt <= ATTEMPT_COUNT_TO_DECODE_BITMAP; attempt++) {
            try {
                result = decoder.decode(imageLoadingInfo.targetSize, imageLoadingInfo.options.getImageScaleType(),
                        imageLoadingInfo.imageView.getScaleType());
            } catch (OutOfMemoryError e) {
                Log.e(com.warriormedia.library.imageloader.core.ImageLoader.TAG, e.getMessage(), e);
