...
import com.wrriormedia.library.imageloader.utils.FileUtils;
...
final class LoadAndDisplayImageTask implements Runnable {
	...
	private final com.wrriormedia.library.imageloader.core.ImageLoaderConfiguration configuration;
	private final ImageLoadingInfo imageLoadingInfo;
	private final Handler handler;

	public LoadAndDisplayImageTask(com.wrriormedia.library.imageloader.core.ImageLoaderConfiguration configuration, ImageLoadingInfo imageLoadingInfo,
			Handler handler) {
	...
	public void run() {
		if (configuration.loggingEnabled)
			Log.i(com.wrriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_START_DISPLAY_IMAGE_TASK, imageLoadingInfo.memoryCacheKey));
		...
		if (imageLoadingInfo.options.isCacheInMemory()) {
			if (configuration.loggingEnabled)
				Log.i(com.wrriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_CACHE_IMAGE_IN_MEMORY, imageLoadingInfo.memoryCacheKey));
		...
		if (checkTaskIsNotActual())
			return;
		if (configuration.loggingEnabled)
			Log.i(com.wrriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_DISPLAY_IMAGE_IN_IMAGEVIEW, imageLoadingInfo.memoryCacheKey));
	...
	boolean checkTaskIsNotActual() {
		String currentCacheKey = com.wrriormedia.library.imageloader.core.ImageLoader.getInstance().getLoadingUriForView(imageLoadingInfo.imageView);
	...
	private Bitmap tryLoadBitmap() {
		...
		try {
			...
			if (imageFile.exists()) {
				if (configuration.loggingEnabled)
					Log.i(com.wrriormedia.library.imageloader.core.ImageLoader.TAG,
			...
			// Load image from Web
			if (configuration.loggingEnabled)
				Log.i(com.wrriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_LOAD_IMAGE_FROM_INTERNET, imageLoadingInfo.memoryCacheKey));

			URI imageUriForDecoding;
			if (imageLoadingInfo.options.isCacheOnDisc()) {
				if (configuration.loggingEnabled)
					Log.i(com.wrriormedia.library.imageloader.core.ImageLoader.TAG, String.format(LOG_CACHE_IMAGE_ON_DISC, imageLoadingInfo.memoryCacheKey));
			...
			Log.e(com.wrriormedia.library.imageloader.core.ImageLoader.TAG, e.getMessage(), e);
			fireImageLoadingFailedEvent(FailReason.IO_ERROR);
			if (imageFile.exists()) {
				imageFile.delete();
			}
